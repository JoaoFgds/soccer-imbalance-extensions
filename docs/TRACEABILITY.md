...
| 4 | Are observed schedules more imbalanced than feasible counterfactuals? | Some real leagues lie in the upper tail of valid round-order permutations. | Gives SSB an operational null distribution instead of an arbitrary cutoff. | Team-season/permutation; all usable league-seasons. |
...
| 3 | Points per game | `SSB × relative market value × league Gini`; league/year effects | Team-season canonical key | Cluster-robust interaction model; marginal effects, leave-one-league-out, functional form, hierarchical and alternative specifications, joint support; fixed-start Elo, league-cluster, temporal holdout and preceding-season sensitivities | Advance observationally if coverage/support pass, at least 80% of league exclusions retain the sign, at least half of temporally safe alternatives retain it, the hierarchical, holdout, conservative-cluster and fixed-start-Elo signs agree, and a supported marginal effect reaches ±0.10 PPG. Causal or pre-season language additionally requires dated valuation provenance. |
| 4 | Mean absolute SSB | Observed order versus global and phase-preserving round permutations under market-value and fixed-start-Elo strength | League-season, round, fixture, team | 10,000 seeded draws per source/null; empirical tail probability, Monte Carlo SE, FDR control and cross-source agreement | Advance as a benchmark if each strength source covers at least 250 league-seasons, every requested draw completes and maximum Monte Carlo SE is at most 0.0051. Individual-season claims require agreement across strength sources. |
